...
actor Classifier {
    ...
    private func buildStage1Prompt(
        ...
        - area: 지속적으로 참고하는 문서 — 개인 기준/원칙, 반복 업무 워크플로우, 관심 분야 지식, 운영/관리 문서, 습관/루틴, 자주 쓰는 가이드
        - resource: 가끔 참고하는 문서 — 일회성 분석, 특정 기술 레퍼런스, 비정기 학습 자료, 외부 아티클 요약, 스크랩/클리핑
        - archive: 완료된 작업, 오래된 내용, 더 이상 활성이 아닌 문서

        ⚠️ area vs resource 판단: "이 문서를 주기적으로 다시 볼까?" → area, "필요할 때만 찾아볼까?" → resource
        ⚠️ 자주 쓰는 가이드, 워크플로우, 체크리스트, 개인 기준/원칙 문서는 area입니다 (반복 참조).
        ⚠️ 포트폴리오, 이력서, 프로젝트 소개/설명/개요 문서는 resource입니다 (직접 작업 문서가 아님).
        ⚠️ 프로젝트에 대한 일회성 분석/리뷰/회고는 resource 또는 archive입니다.
