# AI Companion Files Enhancement Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** DotBrain이 PKM 볼트에 생성하는 AI 동반 파일(CLAUDE.md, AGENTS.md, .cursorrules)을 대폭 강화하고, .claude/agents/ 및 .claude/skills/ 디렉토리를 추가하여 Claude Code, Cursor, OpenClaw가 PKM을 효과적으로 탐색·활용할 수 있도록 한다.

**Architecture:** 현재 PKMPathManager.createAICompanionFiles()에 하드코딩된 문자열을 `AICompanionService`로 분리하고, CLAUDE.md를 150행→400+행으로 확장, .claude/agents/ 3개 + .claude/skills/ 1개 에이전트 파일을 추가 생성한다. 기존 initializeStructure() 호출 체인은 유지.

**Tech Stack:** Swift 5.9, macOS 13+ (기존 DotBrain 스택)

---

## Task 1: AICompanionService 생성 + 강화된 CLAUDE.md

**현재 문제:** PKMPathManager.createAICompanionFiles()에 3개 파일 내용이 모두 하드코딩 (140행). CLAUDE.md는 ~60행으로 폴더 구조만 설명하고, 검색 패턴/분류 알고리즘/프론트매터 필드별 가이드/탐색 우선순위가 없다.

**목표:** 별도 `AICompanionService`를 만들어 CLAUDE.md를 AI-PKM 수준(400+행)으로 강화한다.

**Files:**
- Create: `Sources/Services/AICompanionService.swift`

**Step 1: Create AICompanionService with enhanced CLAUDE.md**
...
enum AICompanionService {
    ...
    static func generateClaudeMd(pkmRoot: String) throws {
    ...
    **이 폴더 안에서 코드를 작성하지 마세요!**
    ...
    // MARK: - AGENTS.md (continued in Task 4)
    // MARK: - .cursorrules (continued in Task 4)
    // MARK: - .claude/agents/ (continued in Task 2)
    // MARK: - .claude/skills/ (continued in Task 3)
}
```
...
git add Sources/Services/AICompanionService.swift
git commit -m "feat: create AICompanionService with enhanced CLAUDE.md (400+ lines)"
```

---

## Task 2: .claude/agents/ 에이전트 파일 생성

**현재 문제:** DotBrain이 생성하는 PKM에 에이전트 파일이 없어서 Claude Code가 인박스 분류/프로젝트 관리/검색 워크플로를 모른다.

**목표:** `.claude/agents/` 디렉토리에 inbox-agent.md, project-agent.md, search-agent.md 3개 파일을 생성한다.

**Files:**
- Modify: `Sources/Services/AICompanionService.swift` (generateClaudeAgents 메서드 추가)

**Step 1: Add generateClaudeAgents method to AICompanionService**

`Sources/Services/AICompanionService.swift`에 다음 메서드와 문자열 상수를 추가한다.

```swift
    // MARK: - .claude/agents/

    static func generateClaudeAgents(pkmRoot: String) throws {
        let fm = FileManager.default
    ...
    **바이너리 파일 (PDF, PPTX, XLSX, 이미지):**
    ...
    1. 이름 파싱 → 폴더명 생성 (특수문자 제거, PascalCase 또는 snake_case)
...
cd /Users/hwaa/Documents/DotBrain
git add Sources/Services/AICompanionService.swift
git commit -m "feat: add .claude/agents/ with inbox, project, search agent definitions"
```

---

## Task 3: .claude/skills/ 바이너리 처리 스킬 문서 생성

**현재 문제:** AI 도구가 PDF/PPTX/XLSX 파일을 만나면 어떻게 텍스트를 추출할지 모른다.

**목표:** `.claude/skills/inbox-processor/SKILL.md`를 생성하여 바이너리 파일 처리 방법을 문서화한다.

**Files:**
- Modify: `Sources/Services/AICompanionService.swift` (generateClaudeSkills 메서드 추가)

**Step 1: Add generateClaudeSkills method**

```swift
    // MARK: - .claude/skills/

    static func generateClaudeSkills(pkmRoot: String) throws {
        let fm = FileManager.default
...
cd /Users/hwaa/Documents/DotBrain
git add Sources/Services/AICompanionService.swift
git commit -m "feat: add .claude/skills/inbox-processor/SKILL.md for binary processing"
```

---

## Task 4: AGENTS.md + .cursorrules 강화

**현재 문제:** AGENTS.md는 ~30행으로 구조만 나열, .cursorrules도 기본 정보만 포함.

**목표:** AGENTS.md에 에이전트 시스템 메타 문서화 추가, .cursorrules에 검색 패턴/분류 가이드 추가.

**Files:**
- Modify: `Sources/Services/AICompanionService.swift` (generateAgentsMd, generateCursorRules 메서드 추가)

**Step 1: Add generateAgentsMd and generateCursorRules methods**

```swift
    // MARK: - AGENTS.md

    static func generateAgentsMd(pkmRoot: String) throws {
        let fm = FileManager.default
    ...
    **이 폴더 안에서 코드 파일을 생성하지 마세요.**
    ...
    static func generateCursorRules(pkmRoot: String) throws {
        let fm = FileManager.default
...
cd /Users/hwaa/Documents/DotBrain
git add Sources/Services/AICompanionService.swift
git commit -m "feat: enhance AGENTS.md and .cursorrules with search patterns and field guide"
```

---

## Task 5: PKMPathManager 연결 — createAICompanionFiles를 AICompanionService로 교체

**현재 문제:** PKMPathManager.createAICompanionFiles()가 여전히 기존의 간단한 버전을 생성한다. 새 AICompanionService로 교체해야 한다.

**목표:** PKMPathManager.initializeStructure()에서 기존 createAICompanionFiles() 호출을 AICompanionService.generateAll() 호출로 교체한다.

**Files:**
- Modify: `Sources/Services/FileSystem/PKMPathManager.swift:67-79` (initializeStructure 메서드)
- Modify: `Sources/Services/FileSystem/PKMPathManager.swift:82-225` (createAICompanionFiles 메서드 삭제)

**Step 1: Replace createAICompanionFiles call with AICompanionService.generateAll**

`Sources/Services/FileSystem/PKMPathManager.swift`의 `initializeStructure()` 메서드에서:
...
**Step 2: Remove the old createAICompanionFiles method**

`Sources/Services/FileSystem/PKMPathManager.swift`에서 `createAICompanionFiles()` 메서드 전체(82-225행)를 삭제한다.
...
git commit -m "refactor: replace createAICompanionFiles with AICompanionService.generateAll"
```

---

## Task 6: 최종 빌드 검증 + 전체 커밋
...
확인 사항:
- AICompanionService가 생성하는 파일 목록:
  - `CLAUDE.md` (~150행 → ~300행)
  - `AGENTS.md` (~30행 → ~70행)
  - `.cursorrules` (~25행 → ~60행)
  - `.claude/agents/inbox-agent.md` (~120행)
  - `.claude/agents/project-agent.md` (~70행)
  - `.claude/agents/search-agent.md` (~55행)
  - `.claude/skills/inbox-processor/SKILL.md` (~50행)
...
git add -A && git commit -m "chore: final verification of AI companion enhancement"
```
...
| Task | 기능 | 영향 범위 | 우선순위 |
|------|------|----------|---------|
| 1 | AICompanionService + 강화된 CLAUDE.md (300+행) | 신규 서비스 | P0 |
| 2 | .claude/agents/ 3개 에이전트 (inbox/project/search) | AICompanionService 확장 | P0 |
| 3 | .claude/skills/ 바이너리 처리 스킬 문서 | AICompanionService 확장 | P1 |
| 4 | AGENTS.md + .cursorrules 강화 | AICompanionService 확장 | P1 |
| 5 | PKMPathManager 연결 (기존 메서드 교체) | PKMPathManager 수정 | P0 |
| 6 | 최종 빌드 검증 | 전체 | P0 |
