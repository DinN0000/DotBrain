...
- **볼트 점검** — 깨진 링크·누락 프론트매터 자동 수정, 변경 노트 시맨틱 링크 갱신, 링크 재선별(문서당 12개 유지), 주제 위키 유지보수까지 한 번에 수행
...
│   ├── images/                      ← 이미지
│   └── videos/                      ← 동영상
├── _Wiki/                           ← 주제 위키 (AI가 여러 노트를 종합한 주제 페이지)
├── .meta/                           ← 기계용 레이어 (노트/주제 인덱스, 볼트 로그)
├── 1_Project/
│   └── MyProject/
│       ├── MyProject.md             ← 폴더 개체 페이지 (자동 생성)
│       └── plan.md
├── 2_Area/
│   └── DevOps/
│       └── monitoring-guide.md
...
### Wiki-links + 자동 종합 — 사람에게는 목차, AI에게는 탐색 그래프

각 폴더에는 **폴더 개체 페이지**(`폴더명.md`)가 자동 유지됩니다.
AI가 폴더의 개요 · 최근 흐름 · 핵심 노트를 종합해 두는 진입점 노트입니다.

여러 폴더에 흩어진 같은 주제는 **`_Wiki/` 주제 페이지**로 종합됩니다.
현재 이해 · 모순 · 노후 · 타임라인 · 멤버 노트 — 새 노트가 들어올 때마다 주제 이해가 갱신되어, 기록이 쌓일수록 지식이 복리로 쌓입니다.
...
`.meta/`에는 노트·주제 인덱스(JSON)와 볼트 변경 타임라인(`log.md`)이 함께 유지되어,
...
>
> 그리고 진짜 핵심은, **이렇게 정리된 볼트를 AI가 읽을 때 성능이 확 올라갑니다.** 구조화된 프론트매터, 자동 인덱스, 관계 유형이 붙은 노트 링크, 주제 위키 덕분에 AI가 맥락을 정확히 파악하고, 필요한 문서를 빠르게 찾아냅니다. 내 지식이 잘 정리될수록 AI가 더 똑똑하게 일하는 구조입니다.
>
