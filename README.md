...
- **Compounding Synthesis:** 새 노트가 들어올 때마다 폴더 페이지와 카테고리 허브를 AI가 종합·갱신하여, 기록이 쌓일수록 지식이 복리로 정리됩니다.
...
2단계 AI 분류
    ...
    ↓
파일 이동 + 프론트매터 주입 + 관련 노트 연결 + 인덱스·폴더 페이지·카테고리 허브 갱신
    ↓
분류 완료
```
...
- **볼트 점검** — 깨진 링크·누락 프론트매터 자동 수정, 변경 노트 시맨틱 링크 갱신, 링크 재선별(문서당 12개 유지), 폴더·카테고리 종합 갱신까지 한 번에 수행
...
│   ├── images/                      ← 이미지
│   └── videos/                      ← 동영상
├── .meta/                           ← 기계용 레이어 (노트 인덱스, 볼트 로그)
├── 1_Project/
│   ├── 1_Project.md                 ← 카테고리 허브 (지형·교차연결·모순 종합)
│   └── MyProject/
│       ├── MyProject.md             ← 하위 폴더 페이지 (개요·최근 흐름·핵심 노트·모순·노후)
│       └── plan.md
├── 2_Area/
│   ├── 2_Area.md                    ← 카테고리 허브
│   ├── DevOps/
│   │   └── monitoring-guide.md
│   └── Unsorted/                    ← 분류 애매한 Area·Resource 신규 노트가 모이는 곳
├── 3_Resource/
│   ├── 3_Resource.md                ← 카테고리 허브
│   ├── Python/
│   │   └── asyncio-patterns.md
│   └── Unsorted/
└── 4_Archive/                       ← 종합 없음 (허브·폴더 페이지 없음)
    └── 2024-Q1/
        └── quarterly-report.md
```
...
사람에게는 Obsidian에서 바로 보이는 메타데이터이고, AI에게는 파싱 한 번으로 분류·검색·요약 정보가 추출되는 구조화된 데이터입니다.

**`tags`·`summary`는 AI가 생성하고 유지합니다.** 본문을 고치면 볼트 점검 때 요약과 태그도 최신 내용으로 다시 생성되어 메타데이터가 낡지 않습니다.
사용자가 지키는 것은 자신이 쓴 **산문 본문**뿐이며, DotBrain은 이 본문을 절대 건드리지 않습니다. 메타데이터를 손수 채우는 노동에서 벗어나고, AI는 일관된 규격의 데이터를 확보합니다.

### 세 겹의 종합 — 노트 위에 폴더, 폴더 위에 카테고리

원본 노트는 그대로 두고, AI가 그 위에 두 겹의 종합 레이어를 쌓습니다. 기록이 쌓일수록 상위 레이어가 다시 정리되어 지식이 복리로 축적됩니다.

- **노트** — 원본. AI는 프론트매터와 `## Related Notes`만 기록하고, 사용자가 쓴 산문 본문은 건드리지 않습니다.
- **하위 폴더 페이지**(`폴더명.md`) — 그 폴더의 개요 · 최근 흐름 · 핵심 노트 · 모순 · 노후를 종합한 진입점.
- **카테고리 허브**(`1_Project.md` 등) — 하위 폴더들 *사이*의 지형 · 교차연결 · 모순을 종합. Project · Area · Resource에서 하위 폴더가 2개 이상일 때 생성되며, Archive는 종합하지 않습니다.

지식의 시간 축은 별도 파일이 아니라 흐름으로 남습니다. 각 페이지의 `## 최근 흐름`이 이전 항목을 이어받아 누적되고(스냅샷이 아니라 누적), 종합이 일어날 때마다 `.meta/log.md`에 `synthesis` 한 줄이 append되어 `grep` 한 번으로 훑을 수 있는 연대기가 됩니다.

**모든 종합은 사용자 트리거 + 해시 게이트입니다.** 인박스 정리와 볼트 점검을 실행할 때만, 그리고 내용이 실제로 바뀐 폴더·카테고리만 다시 종합합니다. 백그라운드 스케줄러도, 매번 전체 재작성도 없습니다.

문서 사이는 `## Related Notes` 섹션이 **관계 유형과 한 줄 맥락**을 담아 자동 연결합니다.
문서당 12개를 넘으면 AI가 가장 가치 있는 링크만 남기고 재선별합니다.

```markdown
## Related Notes

### 선행 지식
- [[DeFi 기초]] — 프로토콜 구조를 이해하려면

### 함께 보기
- [[시장 리포트]] — 시장 전체 트렌드와 비교할 때
```

사람에게 이 링크는 클릭으로 이동하는 목차이고, AI에게는 탐색 그래프의 엣지입니다.
`.meta/`에는 노트 인덱스(JSON)와 볼트 변경 타임라인(`log.md`)이 함께 유지되어,
AI가 볼트 전체를 읽지 않고도 어떤 문서를 먼저 읽을지 판단할 수 있습니다.

**같은 구조가 사람에게는 네비게이션으로, AI에게는 탐색 그래프로 작동합니다.**

### AI Companion Files — 볼트를 AI-ready로

DotBrain은 사용자의 볼트에 `CLAUDE.md`(Claude Code)와 `AGENTS.md`(Codex) AI 컴패니언 파일을 자동 생성합니다.
이 파일들이 있으면 AI 코딩 도구가 볼트를 열었을 때 폴더 구조, 분류 규칙, 태그 체계, 탐색 우선순위를 즉시 파악합니다.
가치 있는 종합·분석 답변은 다시 노트로 저장하도록 유도해, 탐구 결과도 볼트에 쌓입니다.
...
"PoC-Alpha", "Research-Beta", "DotBrain" — 어떤 프로젝트가 진행 중인지는 사용자만 압니다.
...
> 파일을 인박스에 모아두고 정리하면 AI가 내용을 분석해서 PARA 구조로 자동 분류하고, 프론트매터 작성, 관련 노트 연결, 폴더·카테고리 페이지 종합까지 다 해줍니다.
>
...
>
> 그리고 진짜 핵심은, **이렇게 정리된 볼트를 AI가 읽을 때 성능이 확 올라갑니다.** 구조화된 프론트매터, 자동 인덱스, 관계 유형이 붙은 노트 링크, 폴더·카테고리 종합 페이지 덕분에 AI가 맥락을 정확히 파악하고, 필요한 문서를 빠르게 찾아냅니다. 내 지식이 잘 정리될수록 AI가 더 똑똑하게 일하는 구조입니다.
>
> Obsidian 호환이고, Claude Code·Codex용 에이전트도 자동으로 심어줘서 "볼트 점검해줘" 한마디로 전체 건강 검사까지 됩니다.
